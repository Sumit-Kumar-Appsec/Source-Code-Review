...
```
GET  /products        → run showAllProducts()GET  /products/:id    → run showOneProduct()POST /login            → run loginUser()POST /cart/add         → run addToCart()
```
...
```jsx
app.get('/products',function(req, res){// code to show all products runs here   res.send("Here are all the products");});app.get('/products/:id',function(req, res){// :id is a variable, picked up from the URLlet productId= req.params.id;   res.send("Product number: "+ productId);});
```
...
1. Browser sends a request: `GET /products/123`
2. The request reaches the server
3. The **router** checks: "It's a GET request, path is `/products/:id`" → match found
4. Router calls the matching function, and passes `id=123` to it
5. The function fetches product 123's details from the database
6. The response is sent back to the browser (as HTML or JSON)
